...
import cv2
from utilities.misc_utils import load_image, save_image, load_vflow
...
def get_current_metrics(item, args):
    ...
    vflow_items = load_vflow(
        vflow_pred_path, agl=agl_pred, args=args, return_vflow_pred_mat=True, unit="m"
    )
...
def get_city_scores(args, site):
    ...
    vflow_gt_paths = list(glob(os.path.join(args.predictions_dir, site + "*_VFLOW*.json")))
    vflow_gt_paths = [g.replace(args.predictions_dir, args.truth_dir) for g in vflow_gt_paths]
    if vflow_gt_paths == []:
        return np.nan
